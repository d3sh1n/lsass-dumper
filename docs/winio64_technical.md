...
> **重要改动**：返回类型从 `i32` 改为 `usize`（x64 上为 64 位）。原因是 `MmGetPhysicalAddress` 等函数返回 64 位物理地址，`i32` 只能捕获低 32 位，导致地址截断错误。

### 两种调用方式

| 方法 | 返回类型 | 适用场景 |
|------|---------|---------|
| `kernel_syscall()` | `i32`（NTSTATUS） | ZwOpenProcess、ZwClose 等返回状态码的函数 |
| `kernel_syscall_raw()` | `u64`（完整 RAX） | MmGetPhysicalAddress 等返回 64 位值的函数 |
...
> **历史问题**：最初尝试使用 `MmCopyMemory` 将 CR3 直接复制到用户态缓冲区，导致 BSOD。原因是 `MmCopyMemory` 要求目标缓冲区在**非分页内核内存**中，用户态栈地址不满足此条件。改用 `MmGetPhysicalAddress` + `read_phys` 的两步方案安全可靠。
...
### 6.3 x64 四级页表遍历（`translate_va_to_pa`）
...
| NtReadVirtualMemory | 用户态调用 → EDR hook 拦截 | 物理内存 `read_phys` | ✅ 已规避 |
